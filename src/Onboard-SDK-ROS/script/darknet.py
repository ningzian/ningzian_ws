...
def class_colors_green(names):
    """
        Create a dict with one random BGR color for each
        class name
        """
    return {name: (0, 255, 0) for name in names}
...
def load_network(config_file, data_file, weights, batch_size=1):
    ...
    colors = class_colors_green(class_names)
...
hasGPU = True
if os.name == "nt":
    cwd = os.path.dirname(__file__) + '/darknet_file'
    os.environ['PATH'] = cwd + ';' + os.environ['PATH']
    winGPUdll = os.path.join(cwd, "yolo_cpp_dll.dll")  # yolo_cpp_dll
